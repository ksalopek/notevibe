...
## 13. Admin Dashboard & Advanced Inertia Features

We expanded the admin section to be a full-fledged dashboard for managing users and platform content, while introducing some advanced Inertia.js concepts for performance.

### Last Login Tracking
We added a `last_login_at` timestamp to the `users` table to track activity. 
In `AuthenticatedSessionController.php`, right after a successful login and session regeneration, we update this timestamp:
```php
$request->user()->update(['last_login_at' => now()]);
```

### Lazy Evaluation (Partial Reloads)
In the `AdminController`, passing large datasets (like all users or all notes) on every request can be slow. We used **Lazy Evaluation** (closures) for our Inertia props:
```php
'metrics' => fn () => [
...
```
By wrapping the data in a `fn () =>`, Inertia won't execute these database queries unless the frontend explicitly asks for them (using "Partial Reloads"). This makes navigating between admin tabs incredibly fast.

### Search, Sort, and Pagination
We enhanced the Admin tables to support complex server-side data tables:
* **Sorting**: We accept `sort` and `direction` query parameters to dynamically order database results.
* **Reusable Pagination**: We built a custom `Pagination.jsx` component to handle the pagination links generated by Laravel's `->paginate(10)`.

### Global Note Management
Administrators now have a dedicated page to search and manage all notes across the platform. They can view the note's creator (via Eloquent relationships) and have the authority to globally delete any note, bypassing the standard `NotePolicy`.

---

### Keep Learning!
If you want to keep exploring, try adding these features next:
1. **Sharing Notes**: Allow users to share their notes with other users.
2. **Rich Text Editor**: Replace the plain textareas with a modern rich text editor like Tiptap.
