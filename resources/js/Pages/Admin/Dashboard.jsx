        ...
        >
            <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity hidden md:block">
                <Tooltip content="Drag to move">
                    <div 
                        className="p-2 cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 transition-colors bg-white/50 dark:bg-slate-800/50 rounded-lg backdrop-blur-sm dashboard-drag-handle max-lg:pointer-events-none"
                        onMouseDown={() => window.__isDraggingWidget = false}
                        onMouseMove={() => window.__isDraggingWidget = true}
                        onMouseUp={() => setTimeout(() => window.__isDraggingWidget = false, 100)}
                    >
                        <GripVerticalIcon />
                    </div>
                </Tooltip>
